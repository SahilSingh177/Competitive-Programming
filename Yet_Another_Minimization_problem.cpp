...
void compute(ll l, ll r, ll ql, ll qr) {
    if (l > r) return;
    ll md = (l + r) / 2;
    pair<ll, int> ans = {INF, -1};
    for (int j = ql; j <= min(md, qr); j++) {
        adjust(j, md); 
        ll cur = prev_dp[j - 1] + cost;
        if (cur < ans.fi) {
            ans = {cur, j};
        }
    }
    dp[md] = ans.fi;
    ll qm = ans.se;
    compute(l, md - 1, ql, qm);
    compute(md + 1, r, qm, qr);
}
