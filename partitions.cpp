...
long long power(ll base, ll exp) {
    long long res = 1;
    base %= mod;
    while (exp > 0) {
...
void solve(){
    ...
    ll tot = 0;
    rep(i,0,n){
        tot = addmod(tot, arr[i]);
    }
    combi(k);
    ll sm = 0;
    rep(i,1,k+1){
        ll sign = ((k-i)%2==1)?mod-1:1;
        ll prev = power(i,n-1);
        ll j_n = mulmod(prev,i);
        ll val = addmod(j_n,mulmod(n-1,prev));
        ll curr = mulmod(ncr(k,i),val);
        curr = mulmod(curr,sign);
        sm = addmod(sm,curr);
    }
    sm = (sm+mod)%mod;
    sm = mulmod(sm,invFact[k]);
    sm = mulmod(tot,sm);
    print(sm);
}
