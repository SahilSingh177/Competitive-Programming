...
void updateMin(ll pos, ll val, ll start, ll end, ll index, ll seg[])
{
    ...
    ll mid = (start + end) / 2;
    if (pos <= mid)
        updateMin(pos, val, start, mid, 2 * index + 1, seg);
    else
        updateMin(pos, val, mid + 1, end, 2 * index + 2, seg);
    seg[index] = mergeMin(seg[2 * index + 1], seg[2 * index + 2]);
}
...
void updateMax(ll pos, ll val, ll start, ll end, ll index, ll seg[])
{
    ...
    ll mid = (start + end) / 2;
    if (pos <= mid)
        updateMax(pos, val, start, mid, 2 * index + 1, seg);
    else
        updateMax(pos, val, mid + 1, end, 2 * index + 2, seg);
    seg[index] = mergeMax(seg[2 * index + 1], seg[2 * index + 2]);
}
...
void solve()
{
    ...
    rep(i, 1, n+1)
    {
        ...
        while (!q.empty())
        {
            ...
            for (auto v : adj[node])
            {
                ll val = dist[node] ^ v.second;
                if (dist[v.first] == -1)
                {
                    dist[v.first] = val;
                    q.push(v.first);
                }
                else if (dist[v.first] != val)
                {
                    flg = false;
                }
            }
    ...
    else
    {
        ...
        for (auto edge : edges)
        {
            ...
            {
                cout << edge[0] << " " << edge[1] << " " << (dist[edge[1]] ^ dist[edge[0]]) << endl;
            }
        }
    }
}
