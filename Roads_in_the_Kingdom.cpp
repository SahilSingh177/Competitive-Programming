...
void solve(){
    ...
    ll L = W[k-1] + edges[k-1];
    vll pref(k), pref1(k), prefmax(k);
    pref[0] = H[0] + W[0];
    pref1[0] = H[0] - W[0];
    prefmax[0] = D[0];
    rep(i, 1, k) {
        prefmax[i] = max({prefmax[i-1], D[i], H[i] + W[i] + pref1[i-1]});
        pref[i] = max(pref[i-1], H[i] + W[i]);
        pref1[i] = max(pref1[i-1], H[i] - W[i]);
    }
    vll suff(k), suff1(k), suffmax(k);
    suff[k-1] = H[k-1] + W[k-1];
    suff1[k-1] = H[k-1] - W[k-1];
    suffmax[k-1] = D[k-1];
    for(int i=k-2; i>=0; i--) {
        suffmax[i] = max({suffmax[i+1], D[i], H[i] - W[i] + suff[i+1]});
        suff[i] = max(suff[i+1], H[i] + W[i]);
        suff1[i] = max(suff1[i+1], H[i] - W[i]);
    }
    ll mini = prefmax[k-1];    
    rep(i, 0, k - 1) {
        ll cur_dia = max({prefmax[i], suffmax[i+1], pref[i] + suff1[i+1] + L});
        mini = min(mini, cur_dia);
    }
    print(mini);
