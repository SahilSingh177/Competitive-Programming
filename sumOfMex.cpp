...
void pre(ll n){
    rep(k,0,301){
        rep(j,0,k+1){
            ll sm = 0;
            rep(t,0,j+1){
                ll curr = mulmod(ncr(j, t), binExp(n - t, k));
                if(t&1) sm = submod(sm,curr);
                else sm = addmod(sm,curr);
            }
            ways[k][j] = sm;
        }
    }
...
ll find(ll x, vll &nxt){
    if(nxt[x] == x) return x;
    return nxt[x] = find(nxt[x], nxt);
}
...
void solve(){
    ...
    rep(i,0,n){
        if(arr[i]==-1){
            ...
            if(!vis[arr[i]]){
                vis[arr[i]] = true;
                nxt[arr[i]] = find(arr[i]+1, nxt);
            }
        }
        ll curr = find(0, nxt);
        ll ans = 0;
        rep(j,0,k+1){
            ans = addmod(ans,mulmod(curr,ways[k][j]));
            curr = find(curr+1, nxt);
        }
        cout << ans << " ";
    }
    cout<<endl;
