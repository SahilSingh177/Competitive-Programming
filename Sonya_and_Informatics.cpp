...
void solve(){
    ...
    ll tot = mulmod(n,n-1)/2;
    ll tot_inv = modInv(tot);
    Matrix M(z+1);
    rep(x,0,z+1){
        ll inc = mulmod(z-x,z-x);
        ll dec = mulmod(x,n-2*z+x);
        ll same = tot - inc-dec;
        M.a[x][x] = mulmod(same,tot_inv);
        if(x+1<=z){
            M.a[x][x+1] = mulmod(inc,tot_inv);
        }
        if(x-1>=0){
            M.a[x][x-1] = mulmod(dec,tot_inv);
        }
    }
    Matrix m_k = matPow(M,k);
    print(m_k.a[cnt][z]); 
}
