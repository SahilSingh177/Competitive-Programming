...
vll adj[MAXN];
ll up[MAXN][LOG],sz[MAXN],par[MAXN],ans[MAXN],dep[MAXN];
bool vis[MAXN];

void lca(ll u,ll p,ll d) {
    up[u][0] = p;
    dep[u] = d;
    for(int i=1; i<LOG; i++){
        if(up[u][i-1]!=-1){
...
ll dist(ll u, ll v) {
    return dep[u] + dep[v] - 2*dep[f(u, v)];
}
...
ll query(ll u) {
    ...
    ll res = INF;
    while(curr!= -1){
        res = min(res, ans[curr] + dist(u, curr));
        curr = par[curr];
    }
    return res;
}
