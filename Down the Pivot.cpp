...
void combi(int n){
    ...
    for (int i=0;i<n/2; i++) {
        ll num = (fact[2 * i] * invFact[i]) % mod;
        num = (num * invFact[i]) % mod;
        catalan[i] = (num * modInv(i + 1)) % mod;
    }
}
...
ll f(ll n, ll k) {
    ...
    arr[0] = 1;
    rep(i,1,n){
        arr[i] = (2 * arr[i - 1] % mod - ncr(i - 1, k) + mod) % mod;
    }
    ll sm = 0;
    rep(i,0,n){
        ll l = (catalan[i] * arr[i]) % mod;
        ll r = (catalan[n - 1 - i] * arr[n - 1 - i]) % mod;
        sm = (sm + l * r) % mod;
    }
    return sm;
}
