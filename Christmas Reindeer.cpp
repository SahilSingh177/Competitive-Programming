    ...
    while(m--){
        ...
        if(t==1){
            ...
            for(int v = 60; v>=0; v--){
                ...
                if(idx<0){
                    ll rem = pref[v];
                    ans = addmod(ans, mulmod(ways, pw[rem]));
                    ways = 0; 
                    break;
                }
                ll lo = 0;
                ...
                if(val>lo) {
                    ll z=0;
                    for(int k=0; k<=lo; k++){
                        z=addmod(z, ncr(val,k));
                    }
                    ll w=submod(pw[val], z);
                    ll rem = (v>0) ? pref[v-1]:0;
                    ll add = mulmod(mulmod(ways, w), pw[rem]);
                    ans = addmod(ans, add);
                }
                if(val>=lo){
                    ways = mulmod(ways, ncr(val, lo));
                    idx = idx-lo-1;
                } 
                else{
                    ways = 0;
