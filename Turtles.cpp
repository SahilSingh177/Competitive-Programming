...
ll f(ll i,ll j,ll x,ll y,vector<string> &vec){
    ll n = vec.size();
    ll m = vec[0].size();
    if(i>x || j>y) return 0;
...
void solve(){
    ...
    ll t1 = f(1,2,n-1,m,vec);
    ll t2 = f(2,1,n,m-1,vec);
    ll s1 = f(1,2,n,m-1,vec);
    ll s2 = f(2,1,n-1,m,vec);
    ll tot = mulmod(t1,t2);
    ll diff = mulmod(s1,s2);
    print(submod(tot,diff));
