...
void solve() {
    ...
    vll c;
    int tmp1 = 0, tmp2 = 0;
    while(tmp1<p.size() && tmp2<b.size()){
        if(ask(p[tmp1],b[tmp2])){
            c.pb(p[tmp1++]);
        } 
        else{
            c.pb(b[tmp2++]);
    ...
    ll low=0, high = (ll)c.size()-1;
    ll pos = c.size();
    while(low<=high){
        ll md = low + (high - low)/2;
        if(ask(k, c[md])){
            pos = md;
            high = md - 1;
        } 
