...
void solve(){
    inll (n);
    // ll cnt = mulmod(mulmod(mulmod(3,n),mulmod(3,n))/6,2);
    // bool flg = true;
    // ll curr = 1;
    // print(cnt);
    // rep(i,1,3*n){
    ...
    if(n%2==0){
        ...
        for (int c = 0; c < 256;c++)
        {
            if (pts[c].size() == 3)
            {
                cout << pts[c][0].first << " " << pts[c][0].second << " "<< pts[c][1].first << " " << pts[c][1].second << " "<< pts[c][2].first << " " << pts[c][2].second << endl;
            }
        }
