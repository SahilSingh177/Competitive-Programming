...
ll f(ll odd,ll even,ll par,ll turn){
    if(odd==0 && even==0){
        return par==0;
    }
    if(dp[odd][even][par][turn]!=-1) return dp[odd][even][par][turn];
    bool ans;
    if(turn){
        ans = false;
        if(odd>0) ans |= f(odd-1,even,par^1,0);
        if(even>0) ans |= f(odd,even-1,par,0);
    }
    else{
        ans = true;
        if(odd>0) ans &= f(odd-1,even,par,1);
        if(even>0) ans &= f(odd,even-1,par,1);
    }
    return dp[odd][even][par][turn] = ans;
}
