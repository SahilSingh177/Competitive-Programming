...
void solve(){
    ...
    rep(i,1,x){
        rep(j,k,n+1){
            ...
            if((i-j)%2 != 0) continue;
            ll curr = mulmod(ncr(i,j),ncr(x-i,j));
            curr = mulmod(curr,fact[j]);
            if((i-j)%2==1){
                curr = 0;
            }
            curr = mulmod(curr,getdf(i-j-1));
            curr = mulmod(curr,getdf(x-i-j-1));
            cnt = addmod(cnt,curr);
        }
    }
    cnt = mulmod(modInv(getdf(x-1)),cnt);
    ll len = mulmod(l,modInv(x+1));
    cnt = mulmod(len,cnt);
    print(cnt);
}
