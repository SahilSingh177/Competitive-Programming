...
void pre(){
    ...
    rep(i,1,202){
        rep(j,1,i+1){
            st[i][j] = addmod(st[i-1][j-1],mulmod(j,st[i-1][j]));
        }
    }
}
...
void dfs(ll u,ll p){
    ...
    for(int v:adj[u]){
        ...
        dfs(v,u);
        vll ndp(k + 1, 0);
        rep(j,0,min(k,sz[v])+1){
            ndp[j] = dp[v][j];
            if(j>0){
                ndp[j] = addmod(ndp[j],dp[v][j-1]);
            }
        }
        ndp[1] = submod(ndp[1],1);
        rep(j,0,min(k,sz[v])+1){
            ans[j] = submod(ans[j],ndp[j]);
        }
        vll nxt(k+1,0);
        rep(i,0,min(k,sz[u])+1){
            rep(j,0,min(k,sz[v])+1){
                if (i+j<=k){
                    nxt[i+j] = addmod(nxt[i+j],mulmod(dp[u][i],ndp[j]));
                }
            }
        }
        sz[u] += sz[v];
        rep(j,0,min(k,sz[u])+1){
            dp[u][j] = nxt[j];
        }
    }
    rep(j,0,min(k,sz[u])+1){
        ans[j] = addmod(ans[j],dp[u][j]);
    }
}
...
void solve(){
    ...
    ll val = 0;
    rep(j,1,k+1){
        ll term = mulmod(st[k][j],fact[j]);
        term = mulmod(term,ans[j]);
        val = addmod(val,term);
    }
    print(val);
}
