...
void solve(){
    ...
    ll l = find_lcm(k-1);
    ll ans = 0;
    vector<ll> dp(l), ndp(l);
    rep(i,0,n){
        ll q = a/l;
        dp[a%l]++;
        a = (a * x + y) % m;
        ll contrib = mulmod(k, binExp(n, k - 1));
        contrib = mulmod(contrib, mulmod(q, l));
    ...
    rep(j,0,k){
        ...
        for(int v=0;v<l;v++){
            if(dp[v]==0) continue;
            ll cnt = dp[v];
            ll ways = mulmod(cnt, binExp(n, k - j - 1));
            ans = addmod(ans, mulmod(v, ways));
            ndp[v] = addmod(ndp[v], mulmod(cnt, (n - 1)));
            ll nv = v - (v % (j + 1));
            ndp[nv] = addmod(ndp[nv], cnt);
        }
        dp.swap(ndp);
    }
    print(ans);
